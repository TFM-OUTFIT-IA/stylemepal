...
class InferenceEngine:
    def __init__(self):
        ...
        try:
            ...
            self.style_features = self._get_text_features([
                f"a photo of {s.lower()} clothing" for s in self.style_prompts
            ])
            self.weather_features = self._get_text_features([
                f"a photo of {w.lower()} weather clothing" for w in self.weather_prompts
            ])
            ...
            self.ready = True
            logger.info("InferenceEngine initialized and ready")
        except Exception:
            logger.exception("Failed to initialize InferenceEngine")
            # keep ready False; callers should check
    ...
    def process_image(self, image: Image.Image) -> dict:
        if not getattr(self, "ready", False):
            logger.error("InferenceEngine called when not ready")
        ...
        with torch.no_grad():
            ...
            # Pass the 512-D features through the GNN to get 128-D features
            compat_features = self.gnn_model(img_features, edge_index, edge_type)
        ...
        return {
            ...
            "compat_embedding": compat_features.cpu().numpy().tolist()[0],
        }